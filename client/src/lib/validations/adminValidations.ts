...
export const categorySchema = yup.object().shape({
  ...
  displayOrder: yup.number().typeError('Please enter a valid number for display order').required('Display order is required').min(0, 'Order must be 0 or greater'),
});

export const itemSchema = yup.object().shape({
  title: yup.string().required('Title is required').min(2, 'Title must be at least 2 characters'),
  description: yup.string().required('Description is required').min(10, 'Description must be at least 10 characters'),
  price: yup.number().typeError('Please enter a valid price').required('Price is required').positive('Price must be positive'),
...
export const chefSchema = yup.object().shape({
  ...
  photoUrl: yup.string().optional(),
  experienceYears: yup.number().typeError('Please enter a valid number of years').integer().min(0, 'Experience must be 0 or greater').nullable(),
